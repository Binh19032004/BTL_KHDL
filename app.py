...
# ==============================================================================
# CÁC HÀM LẤY DỮ LIỆU, XỬ LÝ VÀ VẼ BIỂU ĐỒ (Giữ nguyên)
# ==============================================================================
...
# --- (Giữ nguyên tất cả các hàm chart_... của bạn) ---
def chart_hist(d):
    fig = go.Figure()
    ret = d['ret'].dropna()
...
def chart_network(d):
    ...
    fig.update_layout(title='Bản Đồ Mô Hình - Lợi Suất Theo Giờ × Ngày', xaxis_title='Giờ Trong Ngày', yaxis_title='Ngày Trong Tuần', height=500)
    return fig
...
def main():
    ...
    # === PHẦN 2: TẤT CẢ CÁC BIỂU ĐỒ (hiển thị tuần tự) ===
    ...
    st.markdown('<div class="insight"><b>🌳 Sơ Đồ Cây - Lợi Suất Theo Ngày (Phân Tích Thời Gian):</b><br>Mỗi hình chữ nhật = 1 ngày. Kích thước hình = lợi suất tuyệt đối (càng to = dao động càng lớn). Màu xanh = ngày tăng (lợi suất dương). Màu đỏ = ngày giảm (lợi suất âm). Xem nhanh ngày nào "sôi động" nhất và ngày nào lợi suất tốt nhất. Điều này giúp phát hiện mô hình giao dịch theo ngày.</div>', unsafe_allow_html=True)
